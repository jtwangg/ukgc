...
class GraphTextCollator(DataCollatorForSeq2Seq):
    ...
    def __init__(self, tokenizer, model=None, padding=True, pad_to_multiple_of=None, return_tensors=None, fp16=False, bf16=False):
        # 调用父类的构造函数，传入所有必要的参数
        super().__init__(
            tokenizer=tokenizer,
            model=model,
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for feature in features:
            ...
            confidence_data = feature.pop("confidence", None)
            if confidence_data is not None:
                confidence_list.append(confidence_data)
            
            # 剩下的键（input_ids, labels 等）被添加到 text_features
            text_features.append(feature)

        assert len(graph_list) == len(text_features), (
            f"Graph and Text feature lists have mismatched lengths! "
            f"Text Features (should be batch size): {len(text_features)}, "
            f"Graph List (extracted graphs): {len(graph_list)}"
        )
        
        # 此外，text_features 的长度应该等于原始的 batch size
        original_batch_size = len(features)
        assert len(text_features) == original_batch_size, (
            f"Text Features list length ({len(text_features)}) does not match "
            f"original batch size ({original_batch_size}). A feature was dropped."
        )
        ...
        if confidence_list:
            batch['confidence'] = torch.tensor(confidence_list, dtype=torch.float32)
        return batch
...
def main(args):
    ...
    # )
    trainer = CustomTrainer(  # 使用CustomLossTrainer替代Trainer
        model=sp_model,
        args=training_args,
        train_dataset=train_dataset,
        eval_dataset=val_dataset,
        ...
        custom_loss_weight=args.kl_weight,  # 自定义损失的权重，可以根据需要调整
    ...
    with open(path, "w") as f:
        for step, batch in enumerate(test_loader):
            with torch.no_grad():
                output = sp_model.inference_customloss(batch)
