...
def step_one():
    ...
    for i, row in tqdm(dataset.iterrows(), total=len(dataset)):
        ...
        for tri in row['graph']:
            ...
            if t not in nodes:
                nodes[t] = len(nodes)
            edges.append({'src': nodes[h], 'edge_attr': r, 'dst': nodes[t], 'weight': w})
        nodes = pd.DataFrame([{'node_id': v, 'node_attr': k} for k, v in nodes.items()], columns=['node_id', 'node_attr'])
        edges = pd.DataFrame(edges, columns=['src', 'edge_attr', 'dst', 'weight'])
...
def step_two():
    ...
    for index in tqdm(range(len(dataset))):
        ...
        edge_weights = torch.tensor(edges['weight'].tolist(), dtype=torch.float32)  # 转换为 tensor

        pyg_graph = Data(
            x=x,                        # nodes向量
            edge_index=edge_index,      # [head列表, tail列表]
            edge_attr=edge_attr,        # edge向量
            num_nodes=len(nodes),       # 节点数量
            edge_weight=edge_weights,   # 边权重（置信度）
            )
